...
class BeautifulPicture():
    def __init__(self):  #类的初始化操作
        ...
        self.web_url = 'http://meisu.huaban.com/topic.php?id=278'  #要访问的网页地址
        self.folder_path = 'D:\Desktop\BeautifulPicture'  #设置图片要存放的文件目录
    ...
    def get_pic(self):
        ...
        print('开始获取所有img标签')
        all_img = BeautifulSoup(driver.page_source, 'lxml').find_all('img', class_='lazyload')  #获取网页中的class为cV68d的所有a标签
        print('开始创建文件夹')
        is_new_folder = self.mkdir(self.folder_path)  #创建文件夹，并判断是否是新创建
        print('开始切换文件夹')
        os.chdir(self.folder_path)   #切换路径至上面创建的文件夹

        print("a标签的数量是：", len(all_img))  #这里添加一个查询图片标签的数量，来检查我们下拉操作是否有误
        file_names = self.get_files(self.folder_path)  #获取文件家中的所有文件名，类型是list

        for img in all_img: #循环每个标签，获取标签中图片的url并且进行网络请求，最后保存图片
            img_url = img['data-original'] #a标签中完整的style字符串
            print('img标签的url是：', img_url)
            ...
            name_start_pos = img_url.index('&userid=')  #通过找.com/的位置，来确定它之后的字符位置
            name_end_pos = img_url.index('&imgfile=')
